...
- **Один запрос за раз.** Соединение одно, вызовы к одному серверу выстраиваются в
  очередь. Это свойство нижнего слоя, не API: методы уже `&self`, поэтому
  появление мультиплексирования публичную сигнатуру не изменит.
- **Нужен multi-thread рантайм.** В нижнем слое `Proxy::from_bytes` дозванивается
  по TCP через `block_on` внутри десериализации; на current_thread это дедлок.
